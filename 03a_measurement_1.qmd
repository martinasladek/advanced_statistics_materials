...
We are going to analyse scores for the following 23 items designed to measure three facets (subscales) of statistics anxiety[^03a_measurement_1-1] (no items are reverse-scored):

[^03a_measurement_1-1]: These items actually belong to the **Statistics Anxiety Rating Scale (STARS)** [@cruise1985], but — for education purposes — we will pretend we're testing the scale for the first time.
...
Participants responded to each item with a 1 (low) to 5 (high) rating of how anxious they feel in each situation (making this a **Likert scale**[^03a_measurement_1-2]).

[^03a_measurement_1-2]: A **Likert scale** is a response format in which participants indicate their level of agreement or endorsement on an ordered, typically symmetric set of categories (e.g., from strongly disagree to strongly agree).
...
1.  Define the path to your CSV data file using the `here` package — `here()` constructs file paths relative to your project root and avoids hardcoding absolute paths, making your code more portable across computers; the pipe `|>` then passes the file path from `here()` directly into `read_csv()`, making the code cleaner
...
-   **Continuous or ordinal**[^03a_measurement_1-3] (Likert-type) variables — the response scale should tell us this

-   Roughly **linear relationships** among continuous variables / **monotonic relationships**[^03a_measurement_1-4] among ordinal variables — we usually don't need to formally test this, stable, interpretable correlations are often enough[^03a_measurement_1-5]
...
[^03a_measurement_1-3]: **Continuous variables** can take any value along a numeric scale (at least in principle), such as reaction time or test scores. **Ordinal variables** represent ordered categories, where the order matters but the spacing between categories is not assumed to be equal (e.g., Likert-type responses).

[^03a_measurement_1-4]: **Monotonic relationships** are where higher responses on one item tend to be associated with higher (or lower) responses on another in a consistent direction.

[^03a_measurement_1-5]: For example, if relationships are wildly non-linear (e.g., U-shaped), the correlation can be near zero even when a relationship exists, so look for unexpected near-zero correlations.
...
1.  Compute the polychoric correlation matrix for ordinal data — `psych::polychoric` computes the correlation matrix for ordinal data; `$rho` extracts just the correlation matrix from the full output of `polychoric()`, which otherwise includes thresholds and other info
...
```{r}
cors <- stars_corr[ #<1>
  upper.tri(stars_corr, diag = FALSE)] #<2>

c( #<3>
  under_20 = mean(abs(cors) <= .20) * 100, #<4>
  over_80  = mean(abs(cors) >= .80) * 100  #<5>
) #<5>
```

1.  `stars_corr[...]` — when you subset a matrix with a logical matrix, R returns only the values corresponding to `TRUE`; The result is a **vector** of unique correlations (no duplicates, no diagonal)

2.  `upper.tri(stars_corr, diag = FALSE)` — `upper.tri()` creates a logical matrix the same size as `stars_corr` and marks `TRUE` for cells in the upper triangle and `FALSE` elsewhere; `diag = FALSE` ensures the diagonal is excluded (so we don’t include the 1.00 self-correlations)

3.  `c()` stands for *combine* — it creates a named numeric vector containing the results we calculate inside it

4.  `under_20 = mean(abs(cors) <= .20) * 100` — `under_20 =` creates a vector called `under_20`; `abs(cors)` calculates the absolute value (i.e., ignores the signs) of each correlation in `cors`; `abs(cors) <= .20` creates a logical vector: TRUE if the absolute correlation is less than or equal 0.20, FALSE otherwise; `mean(abs(cors) <= .20)` converts the logical vector into a proportion: TRUE counts as 1, FALSE as 0; `mean` gives the proportion of correlations under 0.20 (counting how many TRUEs there are and dividing by the total); `* 100` converts the proportion into a percentage, so 0.3 becomes 30%

5.  `over_80 = mean(abs(cors) >= .80) * 100` — as above, except the vector is named `over_80` and `>= .80` gives the proportion of correlations over or equal to 0.80
...
-   If the determinant is comfortably above zero, there is no serious **multicollinearity**[^03a_measurement_1-6] problem
...
We have already calculated a polychoric correlation matrix for our data and stored it in `stars_corrs`, so we just need to enter that into Base R's `det` function to get the determinant:
...
⚠️ Our determinant is super-tiny — just **0.00000002697537**[^03a_measurement_1-7]!
...
[^03a_measurement_1-6]: **Multicollinearity** occurs when two or more variables are so highly correlated that they provide largely redundant information, making it difficult to estimate their unique contributions or distinguish them statistically.

[^03a_measurement_1-7]: e-08 is scientific notation that means *move the decimal point 8 places to the left*.
...
To get the KMO values for both overall and item-level KMOs, you just enter your data into the `KMO()` function from the `psych` package:
...
In situations where you would want to calculate Bartlett's, you can do that using the `cortest.bartlett()` function, again from `psych` (`n = nrow(stars_data)` specifies the sample size by counting the number of rows in the data).

```{r}
psych::cortest.bartlett(stars_corr, n = nrow(stars_data))
```
...
5.  `fa = "fa"` tells `psych` to do factor analysis (rather than Principle Components Analysis[^03a_measurement_1-8])

[^03a_measurement_1-8]: **Principal Components Analysis (PCA)** s a *data reduction* method, not a latent variable model. Components are weighted composites of observed variables, not latent constructs explaining shared variance, so PCA is about summarising variables, whilst EFA is about modelling shared variance. If your goal is uncovering latent psychological structure, PCA is solving a different problem, so avoid it for scale construction work.
...
-   **`pa` (principal axis factoring)**
    ...
    -   Iteratively estimates communalities[^03a_measurement_1-9] and extracts factors from the reduced correlation matrix
...
[^03a_measurement_1-9]: [What is a communality?]
...
-   MAP looks at **residual correlations after extracting factors** — the minimum indicates the number of factors that best accounts for shared variance without overfitting[^03a_measurement_1-10]
...
[^03a_measurement_1-10]: **Overfitting** occurs when your model captures the data too closely, so it captures not only the true underlying patterns in the data, but also the random noise in your specific dataset. This can make the model look very accurate on the current dataset but perform poorly on new or unseen data.

![](images/scale_development_overfitting.png){fig-align="center" width="300"}
...
1.  **Factor Loadings** (Pattern Matrix)
    -   **MR1** predominantly captures Interpretation Anxiety items with strong loadings (0.55–0.87), but also several Test and Class Anxiety items with moderate loadings (0.34–0.43)
    -   The Test and Class Anxiety items on **MR1** cross-load with similar magnitudes onto **MR2**, suggesting they may relate to more than one factor and need to be reviewed in light of theory
    -   **MR2** captures Test and Class Anxiety items, with four strong loadings (0.65–0.91) and one more moderate loading (0.40)
...
-   **MR1** — all the items designed to reflect Interpretation Anxiety loaded strongly onto MR1, but with some moderately loading Test and Class Anxiety items as well
...
-   **MR2** — all the items designed to reflect Test and Class Anxiety loaded onto MR2, but
    ...
    -   Other, more moderately loading items also loaded (some more strongly) onto MR1, suggesting they also capture whatever MR1 is capturing
...
-   My hypothesis is that MR1 is capturing a general (maybe Statistics Anxiety) factor, MR2 is predominantly capturing Test Anxiety that is not necessarily specific to statistics, and MR3 is predominantly capturing something more akin to Fear of Negative Evaluation, also not necessarily specific to statistics — follow up research should explore these possibilities.
...
**All items designed to reflect *Interpretation Anxiety* loaded strongly onto the first factor (MR1)**, but with **some moderately loaded *Test and Class Anxiety* items which crossloaded onto the second factor (MR2)**.

**Items designed to reflect *Test and Class Anxiety* loaded primarily onto the second factor (MR2)**, with the strongest loadings on exam-related items.

**Items designed to reflect *Fear of Asking for Help* loaded strongly onto the third factor (MR3),** supporting the distinctiveness of this factor. The factor loadings are summarised in Table X.
...
variance_row <- tibble::tibble( #<11>
  Item = "% Variance", #<12>
  `Interpretation Anxiety` = sprintf("%.1f", 
                                     stars_rotated$Vaccounted["Proportion Var", "MR1"]*100), #<13>
  `Test and Class Anxiety` = sprintf("%.1f", 
                                     stars_rotated$Vaccounted["Proportion Var", "MR3"]*100),
  `Fear of Asking for Help` = sprintf("%.1f", 
                                      stars_rotated$Vaccounted["Proportion Var", "MR2"]*100),
  Communality = "" #<14>
)
...
4.  `dplyr::rename()` renames columns to have more meaningful names using `` `New Variable Name` = `Old Variable Name` `` (surround the name with backticks if it has spaces)
...
10. `~ sprintf("%.2f", .x)` (baseR) this is an anonymous (lambda) function — `~` defines the function; `.x` refers to the current column being processed; `sprintf("%.2f", ...)` formats each number to two decimal places; importantly `sprintf()` returns character strings, not numbers, so all formerly numeric columns become character columns formatted for display and doesn't cut of trailing zeros (which is why we don't use `round()`); `%` → format specifier; `.2` → one digit after the decimal point; `f` → fixed-point decimal format
...
13. `` `Interpretation Anxiety` = sprintf("%.1f", stars_rotated$Vaccounted["Proportion Var", "MR1"]*100) `` extract the proportion of variance explained by MR1, convert it to a percentage, round it to one decimal place, and format it as a display-ready string, where `Vaccounted` is a matrix inside the `psych::fa()` object that stores variance information about the factor solution; `["Proportion Var", "MR1"]` this is matrix indexing (which you don't come across when you use `dplyr`): `[row, column]` — here, `"Proportion Var"` selects the row containing the proportion of total variance explained and `"MR1"` selects the column for Factor 1; `* 100` factor analysis reports proportions (0-1), but we usually report percentage values, so we multiply the proportion by 100; `sprintf("%.1f", ...)` we've met before, but we're specifying 1dp instead of 2dp this time, consistent with APA formatting guidelines for percentages
...
17. `setdiff(names(stars_table)[-1], "Communality")` where `names(stars_table)` returns a character vector of all column names in the dataframe; `[-1]` means “drop the first element”; we don't want to bold the first column with the item names; `setdiff(x, "Communality")` a base R function that computes the *set difference* — in other words, "return the elements in `x` that are not in `"Communality"`"
18. `nrow(stars_table)` gets the number of the final row (the variance rows) by counting the number of rows
19. For each loading column, this loop bolds the cells (excluding the final variance row) where the numeric value in that column exceeds .30 (see the callout below for more details)
...
23. `part = "body"` applies formatting to main body of the table, not the headersundefined

::: {.callout-note collapse="true"}
#### What does all that loop code mean?

For each loading column, this loop bolds the cells (excluding the final variance row) where the numeric value in that column exceeds .30 (see the callout below for more details)
...
1.  What is the sample size of the DLBS dataset? `r webex_sample_size`

2.  Is the sample size suitable for EFA? `r webex_sample_suitability`

3.  What type of data are the variables in the DLBS dataset? `r webex_data_type`

4.  What is the percentage of correlations under or equal to .20? `r webex_under_20`

5.  What is the percentage of correlations over or equal to .80? `r webex_over_80`

6.  What can we infer from the correlation percentages? `r webex_prop_corrs_infer`

7.  Which of the following do you infer about item clustering from the correlation heatmap? `r webex_clusters`

8.  Do any items stand out as weak or isolated (select all that apply)? `r webex_problem_items`

9.  Are the correlations consistent with monotonic relationships between variables? `r webex_monotonic`

10. What is the overall KMO statistic (round to 2 decimal places)? `r webex_kmo`

11. What can we infer from the overall KMO statistic? `r webex_kmo_infer`

12. On balance, do you think the data is suitable for EFA? `r webex_data_suitability`

**Estimating the Number of Factors to Extract**

13. Which is the most suitable factoring method to apply? `r webex_nfactor_method`

14. How many factors does the scree plot suggest should be extracted? `r webex_n_scree`

15. Is there any evidence of a large underlying general factor? `r webex_gen_fac`

16. How many factors does parallel analysis suggest should be extracted? `r webex_n_pa`

**Factor Extraction & Rotation**

17. How many factors did you extract? `r webex_n_extracted`

18. Which is the most suitable factor rotation method to apply? `r webex_rotation_method`

19. Which items belong to the first factor? `r webex_factor1_items`

20. Which items belong to the second factor? `r webex_factor2_items`

21. Which items belong to the third factor? `r webex_factor3_items`

22. Which items belong to the fourth factor? `r webex_factor4_items`

23. Are there any items that don't belong to any factors? `r webex_stray_item`

24. How much variance do the factors account for overall?\
    `r webex_total_variance` %

25. What is the lowest factor correlation?\
    `r webex_low_factor_cor`

26. What is the highest factor correlation?\
    `r webex_high_factor_cor`

27. What can we infer from the factor correlations?\
    `r webex_factor_cor_infer`

**Theoretical & Substantive Interpretation**

1.  Which construct do you think may be underlying the first factor?\
    `r webex_factor1_construct`

2.  Which construct do you think may be underlying the second factor?\
    `r webex_factor2_construct`

3.  Which construct do you think may be underlying the third factor?\
    `r webex_factor3_construct`

4.  Which construct do you think may be underlying the fourth factor?\
    `r webex_factor4_construct`

5.  What do you infer about cross-loadings?\
    `r webex_cross_loadings`

6.  What do you infer about weak items?\
    `r webex_weak_items_infer`

7.  Why do you infer theoretically about weak items? `r webex_item20`
...
The pattern matrix showed a **clear and interpretable simple structure**. Primary **loadings were substantial**, **cross-loadings** **were minimal**, and the **mean item complexity was low** (M = 1.1), suggesting that most items loaded strongly onto a single factor.

Factor score adequacy indices were also strong: **correlations between regression-based factor scores and the latent factors ranged from 0.88 to 0.92**, and the **multiple R² values ranged from 0.77 to 0.85**, indicating that the estimated scores represent the underlying constructs well.

...
Before jumping into the GzLM, let's run through a quick recap of the GLM, how it works and are its limitations. Most of this should be familiar from *An Adventure in Statistics* - if you're feeling confident about the ins and outs of the Ordinary Least Squares (OLS) estimation and robust estimation, you can read the [Scenario](#scenario-gzlms) below and then jump straight the section [Making predictions](#making-predictions).
...
>
> H~1~: Belayers using an assisted breaking device will be faster to catch falls.
>
> H~2~: More experienced belayers will be faster to catch falls.
...
3.  Add the mean and confidence intervals
4.  Scale the y axis to reflect the realistic scale of our outcome variable (the maximum value 1030.5 is based on values from @tbl-gzlms-describe
...
Now let's repeat the same for the predictor for H~2~:
...
3.  Add the line of best fit - this time, the predictor is continuous so we use `stat_smooth` instead of `stat_summary`
4.  Scale the y axis and the x axis
5.  Add meaningful labels
6.  Add a theme
...
The nice thing about modelling relationships as straight lines is that once we know the intercept and the slope, we can pretty much extend the line in either direction and make predictions for values that are not represented in your dataset. Consider @fig-gzlms-h2 . Although the line only spans the range of the data, we could extend each end to make predictions.
...
GLM uses Ordinary Least Squares (OLS) estimation to fit models. The goal of OLS is to find the values of $\beta_0$, $\beta_1$, $\beta_2$ ... so that the sum of squared errors (SSE) is as small is it can be given the data provided. Consider @fig-gzlms-h2 : if we were to measure the distance of each point from the line (i.e. the residual), square each distance and then sum all of them together, we'd get an SSE. For the model depicted in @fig-gzlms-h2, the line is the most optimal line that gives the smallest possible SSE.
...
GzLM use *Maximum Likelihood Estimation* (ML)[^01_gzlms-2]. ML tries to find the values of the the parameter estimates where a given *assumed distribution* is the most likely. In other words, where the *likelihood* of that distribution is at its *maximum*.
...
Although the estimates themselves are the same, the confidence intervals and the *p*-values are not. This is deliberate. When we fit OLS models, we use the critical *t* statistic[^01_gzlms-3] based on the residual degrees of freedom to construct the CIs. The test statistic is then calculated by dividing the parameter estimate by its standard error, and this is compared against a theoretical *t* distribution to calculate the *p*-value.

[^01_gzlms-3]: This is not the same as the t statistic that we see in the tables.
...
lines <- readLines("01_gzlms.qmd")

code_lines <- tibble::tibble(
  lines = lines,
...
writeLines(code_lines, "code/01_gzlms_code.qmd")
```
